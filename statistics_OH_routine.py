...
def zonal_average(mds, var=None, dlat=20):
    ...
    if var == 'ver':
        #filter out some ver profiles
        ver_cond = (mds.A_diag>0.8)*(mds.mr>0.8)*(mds.error2_retrieval<3e6)
        groups = mds.ver.where(ver_cond).groupby_bins(
...
if __name__ == '__main__':
    ...
    def clima_OH(year_int):
        ...
        filename = 'daily_ver_clima_{}.nc'.format(year)
    ...
    def clima_agc(year_int):
            ...
            try:
                with xr.open_mfdataset([path_agc+f for f in orbits_agc]) as agc_ds:
                    ...
                    with xr.open_mfdataset([path_ver+f for f in orbits_ver]) as ver_ds:
                        ver_ds = ver_ds.sel(time=agc_ds.time)
                    ...
                    agc_ds = agc_ds.where(cond_ver * cond_min_z * cond_max_z 
        ...
        cond_ver_year = xr.concat(cond_ver_year, dim='time')
        ...
        cond_ver_year.to_netcdf(path+filename, mode='a')
