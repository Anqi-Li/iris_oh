...
ds_limb.close()
year = ds_limb.time.dt.year[0].item()
month = ds_limb.time.dt.month[0].item()
...
ds.close()
year = ds.time.dt.year[0].item()
month = ds.time.dt.month[0].item()
...
# %% open VER file
path_ver = '/home/anqil/Documents/sshfs/oso_extra_storage/VER/Channel1/nightglow/v2/{}{}/'.format(year, str(month).zfill(2))
filename_ver = 'iri_ch1_ver_{}.nc'.format(str(orbit).zfill(6))
...
h_z.rename('Num. of sample').plot(ax=ax, vmax=18, cmap=my_cmap)
...
tan_low = 60e3
tan_up = 95e3
peak_apprx = 5e3/0.55*np.pi*4  # approximation of the airglow peak, used in Sa
...
plt.show()
# %% access AVK 
from VER_invert_routine_v2 import invert_orbit_ch1
# path_limb = '/home/anqil/Documents/sshfs/oso_extra_storage/StrayLightCorrected/Channel{}/'.format(ch)
result_1d, AVK = invert_orbit_ch1(path_limb+filename_limb, save_file=False, im_lst=[0], return_AVK=True)
...
#%%
    ...
    #shift NH 6 months
    mds_nh = mds.sel(latitude_bins=slice(80,20)).roll(doy=180, roll_coords=False)
    mds_sh = mds.sel(latitude_bins=slice(0,-80))
