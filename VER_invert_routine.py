...
#%%
def invert_1d(orbit, ch, path, save_file=False, ver_file_pattern=None, im_lst=None, return_AVK=False):
    ...
    for i in range(len(im_lst)):
        ...
        y = l1.isel(**isel_args).reindex_like(h).values /0.55
        Se = np.diag((error.isel(**isel_args).reindex_like(h).values/0.55)**2)
